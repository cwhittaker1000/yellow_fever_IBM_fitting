...
define('baseapp/annotation-engine', ['jquery', 't5/core/events', 't5/core/dom', 't5/core/forms', 't5/core/forms', 't5/core/zone'], function (jQuery, events, dom, forms, zone) {
  ...
  */
	(function ($, window, document, $proto) {
		...
		function submitViewAnnotationForm() {
			setTimeout(function () {
				if (!Prototype.Browser.WebKit) {
					// We need setTimeout() otherwise it won't be an AJAX form submission as it should
					dom($proto('viewAnnotationForm')).trigger("submit");
				} else {
					// Copied from dom.js, fireNativeEvent()
					//$proto('viewAnnotationForm').submit();
					var element = $proto('viewAnnotationForm');
   ...
   */
		...
		$ae.init = function (options) {
			...
			if (htmlTag.hasClass('ie9') || htmlTag.hasClass('lt-ie9') || htmlTag.attr('data-useragent') && htmlTag.attr('data-useragent').indexOf('Edge') !== -1 || htmlTag.attr('data-useragent') && htmlTag.attr('data-useragent').indexOf('MSIE') !== -1 || htmlTag.attr('data-useragent') && htmlTag.attr('data-useragent').indexOf('Trident/7.0') !== -1) {
				_isIe = true;
			} // hacky workaround for bug/ticket #3559

			var root, button, popup, counter;
			...
			if (_personalizationSupport) {
				...
				root.on('click', '.annot', function () {
					...
					if (link.length) {
						...
						if ($ae.hasSelectionSupport() && $ae.getFixedSelection().isCollapsed) {
							...
							if (!$ae.popupActivated || String(annotationIdsInput.val()) !== String(annotationIds)) {
								annotationIdsInput.val(annotationIds);
								submitViewAnnotationForm();
							}
							return false;
						}
					}
				...
				popup.on('touchend click', '.delete', function () {
					...
					$(this).prev('.deleteLink').on('click', function () {
						window.location.href = this.href;
						return false;
					}).trigger('click');
					return false;
				});
				$proto('viewAnnotationZone').observe(events.zone.didUpdate, function (event) {
					var pathURL = window.location.href;
					var paramURL = pathURL.split('?')[1];
					if (paramURL) {
						if (paramURL.indexOf('editAnnotation') !== -1) {
							//$ae.showPopup('edit', $(this).closest('.annotation'));
							$ae.showPopup('edit');
						} else {
							$ae.showPopup('view');
		...
		$ae.updateZoneContent = function (options) {
			if (_personalizationSupport) {
				...
				var processSubmitEvent = function processSubmitEvent() {
		...
		$ae.getSelection = function (callback) {
			var selection,
			    ...
			    tempStart,
			    reversedPosition,
			    reversePosition = function reversePosition() {
			...
			if (!selection.isCollapsed) {
				if (_personalizationSupport) {
					...
					reversedPosition = $ae.comparePosition(startElement, endElement) === Node.DOCUMENT_POSITION_PRECEDING;
					if (reversedPosition) {
						reversePosition();
					...
					if (end === 0 || $(endElement).text().length === 0) {
						var candidates = document.getElementById('contentRoot').getElementsByTagName('*');
						for (i = 0; i < candidates.length; i++) {
							if ($.trim(candidates[i].innerHTML).length > 0 && selection.containsNode(candidates[i], false)) {
								if ($.trim(selection.toString()).indexOf($.trim($(candidates[i]).text())) > $.trim(selection.toString()).indexOf($.trim($(endElement).text()))) {
									endElement = candidates[i];
								}
							}
						}
					}
					// work around for inline DOM elements that only count for the length of the container element not the entire selection
					if (end < start) {
						var selectionLen = selection.toString().length;
						end = selection.anchorOffset + selectionLen;
					}
		...
		$ae.getXPath = function (node) {
			if (_personalizationSupport) {
				...
				for (; node.length; node = $(node).parent()) {
					nodeId = node.attr('id'), nodeClass = node.attr('class'), nodeName = node[0].nodeName.toLowerCase();
      ...
      **/
					...
					if (nodeId !== undefined && nodeId !== '') {
						...
						for (i = 1, sib = node.prev(); sib.length; sib = sib.prev()) {
							...
							if (sib[0].nodeName.toLowerCase() === nodeName && (nodeName !== 'p' || !$(sib[0]).is(':empty'))) {
								i++;
							}
						}
   ...
   */
		$ae.comparePosition = function (a, b) {
			...
			return a.contains ? (a != b && a.contains(b) && 16) + (a != b && b.contains(a) && 8) + (a.sourceIndex >= 0 && b.sourceIndex >= 0 ? (a.sourceIndex < b.sourceIndex && 4) + (a.sourceIndex > b.sourceIndex && 2) : 1) + 0 : 0;
		};
			...
			if (_personalizationSupport) {
				...
				if (state === 'create') {
					...
					if (annotation) {
						...
						var deleteLink = annotation.find('.deleteLink').clone();
  ...
  */
	(function ($, window, document) {
		...
		$de.stopDrag = function (popup) {
			$(document).off('mousemove.dragannot touchmove.dragannot mouseup.remove_dragannot touchend.remove_dragannot');
			popup.css('opacity', '1');
		};
		...
		$de.keepPopupInSightWhenResizingWindow = function (popup, options) {
			...
			$(window).on('resize.dragannot' + popup.attr('id'), function (e) {
				...
				$de.movePopup(popup, originalTop, 0, win.height() - options.dragTresholdInBrowserEdges, originalLeft, options.dragTresholdInBrowserEdges - popup.outerWidth(), win.width() - options.dragTresholdInBrowserEdges);
			});
		};
		...
		$de.showPopup = function (popup, reposition, options, margin) {
			...
			if (reposition) {
				if (margin) {
					popup.css({ margin: margin, opacity: '1' });
				} else {
   ...
   */
		...
		$de.isCursorInViewPort = function (cursorY, cursorX) {
			var doc = $(document),
			    ...
			    topOffset = doc.scrollTop(),
			    leftOffset = doc.scrollLeft();
			return cursorY >= topOffset && cursorY <= win.height() + topOffset && cursorX >= leftOffset && cursorX <= win.width() + leftOffset;
		};
		...
		$de.stopResize = function () {
			$(document).off('mousemove.resizeannot mouseup.remove_resizeannot touchmove.resizeannot touchend.remove_resizeannot');
		};

		// make object global
		window.POPUP_ENGINE = $de;
		window.POPUP_ENGINE.setDefaults();
	})(jQuery, window, document);
	...
	function scrollToAnnotation(annotId) {
		...
		var target = $j('.annot[data-ids]').filter(function () {
			var ids = $j(this).attr('data-ids').split(',');
			var index = ids.indexOf(String(annotId));
			return index > -1;
		});
	...
	var preventDoubleSubmission = function preventDoubleSubmission() {
		jQuery('#annotationsForm').submit(function () {
	...
	return {
		'init': window.ANNOTATION_ENGINE.init,
		'POPUP_ENGINE': window.POPUP_ENGINE,
		'scrollToAnnotation': scrollToAnnotation,
		'preventDoubleSubmission': preventDoubleSubmission
	};
});
