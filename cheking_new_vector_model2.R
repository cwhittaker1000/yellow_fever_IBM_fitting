...
N <- 100 # this seemed to be working okay when N was 100 but maybe red herring
v_population <- 10000
R0 <- 1
...
for (i in 1:25) {
  ...
  if (i == 1) {
    plot(epi$timestep, x$result$Dobs_count + x$result$D_unobs_count, type = "l", ylim = c(0, N))
  } else {
    par(new=TRUE)
    plot(epi$timestep, x$result$Dobs_count + x$result$D_unobs_count, type = "l", ylim = c(0, N))
  }
